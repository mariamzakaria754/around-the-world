const SearchInput = ({ countriesList, filteredCountriesList }) => {
  const handlerSearch = (e) => {
    e.preventDefault();
    const searchTarm = e.target.elements.search.value;
    const filteredCountries =
      !searchTarm || searchTarm === ""
        ? countriesList
        : countriesList.filter((country) =>
            country.name.official
              .toLowerCase()
              .includes(searchTarm.toLowerCase()),
          );
    filteredCountriesList(filteredCountries);
  };
  return (
    <form className="relative flex-1" onSubmit={handlerSearch}>
